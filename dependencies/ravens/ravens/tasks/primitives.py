...
class PickPlace():
  ...
  def __call__(self, movej, movep, ee, pose0, pose1):
    ...
    # if pick_action:
    #   if ee.check_grasp():
    #     print("Object already picked")
    #     assert False
    #     return False

    #   pick_pose = np.array(pose0).copy()

    #   # Execute picking primitive.
    #   pick_pose[0][-1] = self.height

    #   ee.release()
    #   timeout = movep(pick_pose)
    
    #   # Activate end effector, move up, and check picking success.

    #   delta =-0.001
    #   targ_pose = pick_pose.copy()
    #   i = 0
    #   while not ee.detect_contact():  # and target_pose[2] > 0:
    #     targ_pose[0][-1] += delta #utils.multiply(targ_pose, delta)
    #     timeout |= movep(targ_pose)
    #     if timeout:
    #       print("timed out")
    #       return True
    ...
    #   timeout |= movep(pick_pose, self.speed)
    ...
    #   place_pose = np.array(pose0).copy()

    #   place_pose[0][-1] = self.height

    #   timeout = movep(place_pose, self.speed)

    #   delta =-0.001
    #   i=0
    #   targ_pose = place_pose.copy()
    #   while not ee.detect_contact():  # and target_pose[2] > 0:
    #     targ_pose[0][-1] += delta #utils.multiply(targ_pose, delta)
    #     timeout |= movep(targ_pose)
    #     if timeout:
