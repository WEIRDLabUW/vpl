...
class Sorting(Task):
  ...
  def reset(self, env):
    ...
    num_blocks = 4
    colors = [utils.COLORS['blue'], utils.COLORS['blue'], utils.COLORS['red'], utils.COLORS['red']]
    block_sizes = [(0.025, 0.05, 0.05), (0.05, 0.05, 0.05), (0.025, 0.025, 0.025), (0.05, 0.05, 0.05)]
    block_urdfs = ['sort/large_block.urdf', 'sort/block.urdf', 'sort/large_block.urdf', 'sort/block.urdf']
    ...
    self.goals = [
      ([objs[0]], np.ones((1, 1)), [goal_poses[0]], False, True, 'pose', None, 1 / 4),
      ([objs[1]], np.ones((1, 1)), [goal_poses[1]], False, True, 'pose', None, 1 / 4),
      ([objs[2]], np.ones((1, 1)), [goal_poses[2]], False, True, 'pose', None, 1 / 4),
      ([objs[3]], np.ones((1, 1)), [goal_poses[3]], False, True, 'pose', None, 1 / 4),
    ]
    self.task_info = {
      "colors": colors,
